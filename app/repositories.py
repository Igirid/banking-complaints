...
from app.models import AlertRecord, ComplaintIssue, SocialPost
...
class ComplaintRepository:
    """Repository for complaint-related persistence with a simple in-memory fallback."""
    ...
    @classmethod
    def from_default_database(cls) -> "ComplaintRepository":
        Base.metadata.create_all(bind=engine)
        session = Session(bind=engine)
        return cls(session=session)

    @classmethod
    def from_memory_database(cls) -> "ComplaintRepository":
    ...
    def list_social_posts(self, *, limit: int = 100) -> list[SocialPost]:
        if self.session is None:
            return []
        return self.session.query(SocialPost).order_by(SocialPost.id.desc()).limit(limit).all()

    def add_issue(self, *, topic: str, severity: int, sentiment: float, volume: int, trend: float, regulatory_risk: float, business_impact: float, priority_score: float) -> RepositoryResult:
        if self.session is None:
    ...
    def add_alert(self, *, tenant_id: int, title: str, severity: str, issue_topic: str | None = None) -> RepositoryResult:
        if self.session is None:
            return RepositoryResult(success=False, error="No database session configured")
